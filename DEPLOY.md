...
**Если стили не загружаются (страница без оформления):** возможно, Document root указывает на корень проекта, а не на `public`. В панели хостинга задай переменную окружения `BASE_PATH=public/` для PHP (если поддерживается) или убедись, что корень сайта = `public_html/public`.
...
## Пустые категории / «не всё загрузилось»

- **Категории не показываются** — на сервере база создаётся пустой (таблицы есть, данных нет). Нужно один раз скопировать с компьютера файл **storage/database.sqlite** в папку **public_html/storage/** на сервере (через панель или FTP). После этого на сайте появятся категории и товары.
- **Иконки-плейсхолдеры вместо картинок** — обнови страницу с полной перезагрузкой (Ctrl+F5). Если не помогло — проверь, что в **public_html** на сервере есть папки **img** и **public/assets**, и что при открытии в браузере `https://твой-домен.ru/img/logo_aisi_lenta_full.png` картинка отдаётся.

---

## HTTPS (почему не открывается https)

Сайт по умолчанию отдаётся по **HTTP**. Чтобы работал **HTTPS**:

1. **Включи SSL в панели Space Web:** раздел **Домены** или **SSL-сертификаты** → выбери домен **lenta-nerzhavejushhaja-aisi.ru** → включи бесплатный сертификат (Let's Encrypt) или загрузи свой.
2. После включения сертификата открывай сайт по адресу **https://lenta-nerzhavejushhaja-aisi.ru**.
3. В репозитории уже есть файл **.htaccess** в корне: при следующем деплое он зальётся в **public_html**. В нём настроен редирект с HTTP на HTTPS — после включения SSL в панели пользователей с **http://** будет автоматически перекидывать на **https://**.

Если в панели нет раздела про SSL — напиши в поддержку Space Web: «Как включить HTTPS (SSL) для домена lenta-nerzhavejushhaja-aisi.ru?»
